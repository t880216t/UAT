...
import { Collapse, Row, Col, Card, Icon, Steps, DatePicker } from 'antd';
import { Scatter, Pie } from '@/components/Charts';
...
class ImmReport extends PureComponent {
  ...
  handleDateChange = (date, dateString) => {
    ...
    this.queryTaskReportByDate(detailId, dateString);
  };

  queryTaskReportByDate = (id, executeDate) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'task/queryTaskReportByDate',
      payload: {
        id,
        executeDate,
      },
  ...
  render() {
    ...
    const content = (
      <div className={styles.pageHeaderContent}>
        <p>开始时间：{taskResult.startTime}</p>
        <p>结束时间：{taskResult.endTime}</p>
        <p>
          用例统计：
    ...
    const extraContent = (
      <div className={styles.extraImg}>
        <Pie data={result} colors={['#2ecc71', '#e74c3c']} height={160} lineWidth={4} />
      </div>
    );
        ...
        <Row gutter={24}>
          <Col xl={24} lg={24} md={24} sm={24} xs={24}>
            <Card
              ...
              title="执行结果"
              bordered={false}
              extra={<DatePicker onChange={this.handleDateChange} />}
            >
              <Collapse
                bordered={false}
              ...
              >
                {taskResult.testSuites &&
                  taskResult.testSuites.map(item => (
                    <Panel
                      header={
                        <a
                          target="_blank"
                          rel="noopener noreferrer"
                          href={`/case/ui?id=${item.id}&projectId=${taskResult.project}`}
                        >
                          {item.name}
                        </a>
                      }
                      key={item.id}
                      className={[
                        item.status === 'PASS' ? styles.caseSucess : styles.caseFail,
                    ...
                    >
                      ...
                      >
                        {item.testCase &&
                          item.testCase.map(caseItem => (
                            <Panel
                              header={
                                <a
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  href={`/case/ui?id=${caseItem.id}&projectId=${taskResult.project}`}
                                >
                                  {caseItem.name}
                                </a>
                              }
                              key={caseItem.id}
                              className={[
                                caseItem.status === 'PASS' ? styles.caseSucess : styles.caseFail,
                                styles.customPanelStyle,
                              ]}
                            >
                              <Steps direction="vertical" size="small" className={styles.stepBox}>
                                {caseItem.caseSteps &&
                                  caseItem.caseSteps.map((stepItem, index) => {
                                    ...
                                    if (stepItem.capture) {
                                      return (
                                        <Step
                                          ...
                                          title={stepItem.name}
                                          description={stepImage(stepItem)}
                                        />
                                      );
                                    }
                                    return (
