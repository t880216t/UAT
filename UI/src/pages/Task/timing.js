...
class Timing extends PureComponent {
  ...
  render() {
    ...
    const cardTitle = item => (
      <div className={styles.switchContainer}>
        <a
          ...
          href={`/task/ui/timing/detail?${item.id}`}
    ...
    const reportLink = item => {
      if (item.status === 3) {
        return (
          <Button type="link" icon="file-done" onClick={() => this.handleGoReport(item.id)}>
            查看报告
          </Button>
        );
      }
