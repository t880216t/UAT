...
export default class AddTaskPage extends PureComponent {
          ...
          taskCase.forEach(key => {
            listTree.forEach(item => {
              if (
                ...
                item.noteType === 2 &&
                newTargetKeys.indexOf(parseInt(item.id)) === -1
              ) {
  ...
  render() {
    ...
    const proxyList = proxyConfigList.filter(
      item => item.browserType === getFieldValue('browserType'),
    );
        ...
        <Card bordered={false}>
          <Form onSubmit={this.handleSubmit} style={{ marginTop: 8 }}>
            ...
            <FormItem {...formItemLayout} label="浏览器">
              {getFieldDecorator('browserType', {
                ...
                <Radio.Group buttonStyle="solid">
                  ...
                  <Radio.Button value={1} disabled={getFieldValue('valueType') === 2}>
                    Firefox
                  </Radio.Button>
            ...
            <FormItem
              {...formItemLayout}
              label="全局代理"
              style={{ display: getFieldValue('valueType') === 2 ? 'none' : '' }}
            >
              {getFieldDecorator(
                'proxyType',
                {},
