...
export default class CaseProject extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    if (this.props.selectNoteId !== nextProps.selectNoteId) {
      this.queryNextPorpsCaseProjectConfig(nextProps.selectNoteId);
      return true;
    }
  ...
  queryNextPorpsCaseProjectConfig = id => {
    const { dispatch } = this.props;
    dispatch({
      ...
      <Form {...formItemLayout}>
        ...
        <Form.Item label="依赖库">
          {getFieldDecorator('libs', {
            ...
            <Select mode="multiple" placeholder="选择依赖库" onChange={this.handleLibsChange}>
              {libsList &&
                libsList.map(item => (
