...
import { Form, Input, message, Select, Button, Icon, Radio, Modal, Upload } from 'antd';
...
export default class CaseProject extends Component {
  ...
  handleAddGlobalValue = () => {
    ...
    this.setState({
      globalValues: [...globalValues, { id: 'empty', keyName: '', keyValue: '', valueType }],
    });
  };
    ...
    return (
      <Form {...formItemLayout}>
        ...
        <Form.Item label="文件参数" className={styles.paramsContainer}>
          {caseProjectConfig.globalFiles &&
            caseProjectConfig.globalFiles.map(item => (
              <Form key={item.id} layout="inline">
                ...
                <Form.Item>
                  <Button
                    type="link"
                    icon="minus-square"
                    onClick={() => this.handleDeleteFileValue(item.id)}
                  />
                </Form.Item>
              </Form>
            ))}
