    ...
    if (this.state.rightClickItem !== null) {
      try {
        if (document.getElementById('list_right_menu')) {
          if (
            event.target.compareDocumentPosition(document.getElementById('list_right_menu')) !== 10
          ) {
  ...
  queryListData = caseId => {
    ...
    dispatch({
      ...
      this.setState(
        ...
        () => {
          this.setState(
            { caseSteps: caseData.caseStep.concat([{ id: 'empty', values: ['', '', ''] }]) },
            () => [this.dsyncColumns()],
          );
        },
      );
      ...
      dsyncColumn.push({
        ...
        render: (text, record) => (
          <EditCell
            ...
            record={record}
            handleSave={(changeIndex, value, newStepIndexId) =>
