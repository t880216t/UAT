...
export default class FormKeyValuesCell extends React.Component {
  ...
  render() {
    ...
    const children = searchKeywords.map(searchItem => (
      <Option key={searchItem.id} value={searchItem.key_name}>
        {searchItem.key_name}
      </Option>
    ));
    return (
      <div className={styles.cellContainer}>
        ...
        <Form.Item>
          <Button
            type="link"
            icon="minus-square"
            onClick={() => this.props.handleDeleteGlobalValue()}
            style={{ display: item.id === 'empty' ? 'none' : '' }}
          />
        </Form.Item>
      </div>
    );
